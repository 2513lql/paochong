...
//        this.controller = null;
    ...
    public void shortReportLineTo(PrintWriter w) {
        ...
        TreeSet<Entry<Object,Long>> sortedSteps = (TreeSet<Entry<Object, Long>>) map.get("steps");
        if(sortedSteps.size()==0) {
            return;
        }
        w.print(Histotable.entryString(sortedSteps.first()));
        if(sortedSteps.size()>1) {
            Iterator<Entry<Object,Long>> iter = sortedSteps.iterator();
            iter.next();
            w.print(", ");
        ...
        w.print("; ");
        TreeSet<Entry<Object,Long>> sortedProcessors = (TreeSet<Entry<Object, Long>>) map.get("processors");
        w.print(Histotable.entryString(sortedProcessors.first()));
        if(sortedProcessors.size()>1) {
            Iterator<Entry<Object,Long>> iter = sortedProcessors.iterator();
            iter.next();
            while(iter.hasNext()) {
                w.print(", ");
                w.print(Histotable.entryString(iter.next()));
